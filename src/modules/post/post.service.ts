...
export class PostService {
  ...
  async create(userId: number, createPostDto: CreatePostDto, file: Express.Multer.File): Promise<PostEntity> {
  ...
  async findWithAuthor(userId: number, take: number, skip: number): Promise<PostEntity[]> {
  ...
  async deletePost(id: number) {
    ...
    fs.unlink(
      './uploads/post/images/584x342/' + post.image, 
      (err) => {
        if (err) {
          console.error(err);
          return err;
    ...
    fs.unlink(
      './uploads/post/images/original/' + post.image, 
      (err) => {
        if (err) {
          console.error(err);
          return err;
